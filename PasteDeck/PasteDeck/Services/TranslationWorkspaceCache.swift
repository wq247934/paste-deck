...
enum TranslationWorkspaceCache {
    ...
    static func create(sourceText: String) -> UUID? {
        ...
        item.customTitle = "翻译"
        item.translationWorkspaceData = encode(outputs: [])
    ...
    static func save(itemID: UUID, outputs: [TranslationOutput]) {
        let context = ModelContext(AppModelContainer.container)
        guard let item = fetchItem(id: itemID, in: context) else { return }
        item.translationWorkspaceData = encode(outputs: outputs)
        guard save(context) else { return }
        notify(itemID: item.id, kind: .updated)
    }

    /// 读取历史工作区的原文和已缓存卡片；损坏或旧版数据安全退化为空卡片列表。
    static func load(itemID: UUID) -> (sourceText: String, outputs: [TranslationOutput])? {
        let context = ModelContext(AppModelContainer.container)
        ...
        return (sourceText, decode(item.translationWorkspaceData)?.outputs ?? [])
    }
    ...
    private static func encode(outputs: [TranslationOutput]) -> Data? {
        try? JSONEncoder().encode(TranslationWorkspaceSnapshot(outputs: outputs))
    }
